	...
	public static void main(String[] args) {
		...
		for (Integer k : cmp.keySet()) {
			if (!cmp.get(k).equals(stud.retrieve(k))) {
				System.out.println(
						"ERROR: Key " + k + " was inserted but not found OR has incorrect value in your hash table");
				System.exit(-1);
			}
		}
		System.out.println("DONE");
		...
		for (Integer k : cmp.keySet()) {
			if (!cmp.get(k).equals(stud.retrieve(k))) {
				System.out.println("ERROR: Key " + k
						+ " was inserted and updated but not found OR has incorrect value in your hash table");
				System.exit(-1);
			}
		}
		System.out.println("DONE");
